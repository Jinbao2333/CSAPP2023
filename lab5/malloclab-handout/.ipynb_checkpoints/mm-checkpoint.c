...
int mm_check()
{
    ...
    for (int i = 0; i < 20; i++)
    {
        searchPtr = remove_freelist[i];
        if (searchPtr == NULL)
            continue;
        //只要能够完成循环遍历则证明空闲链表中的指针是否均指向有效的空闲块
        while (searchPtr != NULL)
        { //是否有连续的空闲块没有被合并？
            insert_ptr = searchPtr;
            //只要能正常获取值（prev succ）则证明空闲链表中的指针是否均指向有效的空闲块
            if (!GET_ALLOC(HDRP(NEXT_BLKP(insert_ptr)))
                || !GET_ALLOC(HDRP(PREV_BLKP(insert_ptr))))
            {
                printf("WARNING: Some ajacent blk blocks are unmerged.");
                return 0;
            }

            if (GET_ALLOC(HDRP(insert_ptr)))
            { //空闲列表中的每个块是否都标识为free（空闲）？
                printf("WARNING: A blk block is not free but in list.");
                return 0;
            }
            blk_cnt += 1;
            searchPtr = PRED_POINT(searchPtr);
        }
    }
    //只要完成遍历则证明堆块中的指针是否指向有效的堆地址
    printf("pointer: %p", heap_listp);
    ...
    if (blkCnt != blk_cnt)
    { //是否每个空闲块都在空闲链表中？
        printf("WARNING: Not every blk block is in the list.");
        return 0;
    }
